...
export default function Home() {
  ...
  return <main>
    ...
    <header>
      ...
      <nav>{[["shop","本草集市"],["points","健康积分"],["orders","我的订单"],["merchant","商家中心"],["admin","运营看板"]].map(([k,v])=><button key={k} className={view===k?"active":""} onClick={()=>setView(k as typeof view)}>{v}</button>)}</nav>
      <div className="head-actions"><button className="points-pill" onClick={()=>setView("points")}>叶 {points}</button><button className="cart-btn" onClick={()=>setCartOpen(true)}>购物袋 <b>{cart.reduce((s,x)=>s+x.qty,0)}</b></button><span className="avatar">悦</span></div>
    </header>
