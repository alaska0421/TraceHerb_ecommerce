...
export default function Home() {
  ...
  useEffect(()=>{ fetch("/api/auth/me").then(r=>r.json()).then(d=>{if(d.user){setUser(d.user);setPoints(d.user.points||0);setView(d.user.role==="seller"?"merchant":d.user.role==="admin"?"admin":"shop")}}).catch(()=>{}); },[]);
  ...
  return <main>
    ...
    {view==="admin"&&user?.role==="admin"&&<Admin products={products} setProducts={setProducts} orders={orders} notify={notify}/>}
    ...
    {authOpen&&<AuthModal mode={authMode} setMode={setAuthMode} user={user} onClose={()=>setAuthOpen(false)} onAuth={u=>{setUser(u);setPoints(u.points||0);setView(u.role==="seller"?"merchant":u.role==="admin"?"admin":"shop");setAuthOpen(false);notify(authMode==="register"?"注册成功，欢迎加入迹本草":"登录成功，欢迎回来")}} onLogout={logout}/>}
  </main>
}
...
function Admin({products,setProducts,orders,notify}:{products:Product[];setProducts:(x:Product[])=>void;orders:{amount:number}[];notify:(x:string)=>void}) {
  const [query,setQuery]=useState("");
