...
type Product = { id:number; name:string; category:string; origin:string; price:number; stock:number; sales:number; rating:number; traceCode:string; badge:string; description:string; icon:string; sellerId?:number; storeName?:string; traceDates?:string[] };
type CartLine = Product & { qty:number };
type Toast = { text:string; kind?:"ok"|"warn" };
type Role = "buyer"|"seller"|"admin";
type User = { id:number; username:string; email:string; phone:string; points:number; role:Role; storeName?:string };
...
const defaultTraceDates=(id:number)=>Array.from({length:6},(_,i)=>{
  const date=new Date(Date.UTC(2025,Math.floor((id-1)/3),2+((id*3)%18)+i*(8+(id%4))));
...
export default function Home() {
  ...
  return <main>
    ...
    {view==="shop"&&<>
      ...
      <section id="market" className="market section">
        ...
        <div className="product-grid">{filtered.map(p=><article className="product-card" key={p.id}>
          ...
          <div className="product-info"><small>{p.origin}</small><h3>{p.name}</h3><button className="store-link" onClick={()=>notify(`店铺：${p.storeName||"平台自营"}`)}>店铺 · {p.storeName||"平台自营"}</button><p>{p.description}</p><div className="rating">★★★★★ <span>{p.rating} · 已售 {p.sales}</span></div><div className="price"><strong>¥{p.price}</strong><div><button className="trace-btn" onClick={()=>setTrace(p)}>查溯源</button><button className="add-btn" onClick={()=>add(p)}>＋</button></div></div></div>
        </article>)}</div>
      </section>
    ...
    {view==="merchant"&&user?.role==="seller"&&<Merchant user={user} products={products} setProducts={setProducts} notify={notify}/>}
    ...
    {trace&&<div className="overlay" onClick={()=>setTrace(null)}><section className="modal trace-modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setTrace(null)}>×</button><div className="verified">✓ 数字凭证已核验</div><h2>{trace.name}</h2><p>{trace.origin} · 店铺 {trace.storeName||"平台自营"} · 溯源码 {trace.traceCode}</p><TraceQr product={trace}/><div className="timeline">{traceSteps.map((x,i)=><div key={x}><b>✓</b><span><strong>{x}</strong><small>{(trace.traceDates?.length===6?trace.traceDates:defaultTraceDates(trace.id))[i]} · 记录已上链</small></span></div>)}</div><div className="cert"><b>检测结论：合格</b><span>重金属、农残、二氧化硫等 32 项检测</span></div></section></div>}
...
function AuthModal({mode,setMode,user,onClose,onAuth,onLogout}:{mode:"login"|"register";setMode:(m:"login"|"register")=>void;user:User|null;onClose:()=>void;onAuth:(u:User)=>void;onLogout:()=>void}){
  const [form,setForm]=useState({username:"",email:"",phone:"",account:"",password:"",role:"buyer" as "buyer"|"seller",storeName:""});
  const [error,setError]=useState("");
  const [loading,setLoading]=useState(false);
  const submit=async()=>{
    setLoading(true);setError("");
    try{
      const body=mode==="register"?{username:form.username,email:form.email,phone:form.phone,password:form.password,role:form.role,storeName:form.storeName}:{account:form.account,password:form.password};
...
function Merchant({user,products,setProducts,notify}:{user:User;products:Product[];setProducts:(x:Product[])=>void;notify:(x:string,kind?:"ok"|"warn")=>void}) {
  ...
  return <section className="dashboard section merchant">
    <div className="page-title row"><div><p className="eyebrow">MERCHANT CENTER</p><h1>{user.storeName||`${user.username}的店铺`}</h1><p>仅展示并管理本店商品、库存、订单发货与溯源资料。</p></div><button className="primary" onClick={openCreate}>＋ 发布商品</button></div>
    <div className="metrics">{[["订单总额",`¥ ${orders.reduce((s,o)=>s+o.amount,0).toFixed(2)}`,"本店已支付订单"],["待处理订单",String(pending),"需及时发货"],["本店商品",String(merchantProducts.length),`${merchantProducts.filter(p=>p.stock<70).length} 件库存预警`],["已完成订单",String(orders.filter(o=>o.status==="已完成").length),"买家已确认收货"]].map(x=><article key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><span>{x[2]}</span></article>)}</div>
    <div className="panel merchant-orders"><div className="panel-head"><h2>订单处理</h2><span>只显示包含本店商品的订单</span></div><table><thead><tr><th>订单 / 买家</th><th>本店商品</th><th>金额</th><th>状态</th><th>下单时间</th><th>操作</th></tr></thead><tbody>{orders.map(o=><tr key={o.id}><td><b>{o.id}</b><small>{o.username}</small></td><td>{o.items.map(x=>`${x.name} × ${x.qty}`).join("、")}</td><td>¥{o.amount}</td><td><span className={`status ${o.status}`}>{o.status}</span></td><td>{new Date(o.createdAt).toLocaleString("zh-CN")}</td><td>{o.status==="待发货"?<button className="ship-btn" onClick={()=>ship(o.id)}>确认发货</button>:<span>{o.status}</span>}</td></tr>)}</tbody></table>{!orders.length&&<p className="empty-result">暂无本店订单</p>}</div>
    <div className="panel merchant-products"><div className="panel-head"><h2>本店商品管理</h2><span>新发布商品会永久保存并立即展示给买家</span></div><table><thead><tr><th>商品</th><th>分类 / 产地</th><th>售价</th><th>库存</th><th>销量</th><th>溯源状态</th><th>操作</th></tr></thead><tbody>{merchantProducts.map(p=><tr key={p.id}><td><span className="table-icon">{p.icon}</span><b>{p.name}</b></td><td>{p.category}<small>{p.origin}</small></td><td>¥{p.price}</td><td className={p.stock<70?"low":""}>{p.stock}</td><td>{p.sales}</td><td><span className="verified-pill">✓ 已认证</span></td><td><button onClick={()=>openEdit(p)}>编辑</button></td></tr>)}</tbody></table>{!merchantProducts.length&&<p className="empty-result">本店还没有商品，点击右上角发布第一件商品。</p>}</div>
    {show&&<div className="overlay"><section className="modal checkout product-editor"><button className="close" onClick={()=>setShow(false)}>×</button><p className="eyebrow">{editing?"EDIT PRODUCT":"NEW PRODUCT"}</p><h2>{editing?"编辑商品信息":"发布新商品"}</h2><p className="store-owner-note">上架店铺：{user.storeName||`${user.username}的店铺`}</p><label>商品名称<input value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></label><div className="form-grid"><label>商品分类<select value={form.category} onChange={e=>setForm({...form,category:e.target.value})}><option>药食同源</option><option>滋补药材</option><option>养生茶饮</option><option>手作养生</option></select></label><label>产地<input value={form.origin} onChange={e=>setForm({...form,origin:e.target.value})}/></label></div><label>商品描述<textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label><div className="form-grid"><label>价格<input type="number" min="0.01" value={form.price} onChange={e=>setForm({...form,price:Number(e.target.value)})}/></label><label>库存<input type="number" min="0" value={form.stock} onChange={e=>setForm({...form,stock:Number(e.target.value)})}/></label></div><label>商品标签<input value={form.badge} onChange={e=>setForm({...form,badge:e.target.value})}/></label><fieldset className="trace-date-editor"><legend>溯源流程日期</legend><p>流程固定，请按实际情况填写日期；后一步不得早于前一步。</p><div className="trace-date-grid">{traceSteps.map((step,index)=><label key={step}>{step}<input type="date" value={form.traceDates[index]} max={new Date().toISOString().slice(0,10)} onChange={e=>setForm({...form,traceDates:form.traceDates.map((date,i)=>i===index?e.target.value:date)})}/></label>)}</div></fieldset>{editing&&<p className="trace-code-note">溯源码：{editing.traceCode}（为保证溯源一致性不可修改）</p>}<button className="primary wide" onClick={save}>{editing?"保存修改":"发布并生成溯源码"}</button></section></div>}
  </section>
}
