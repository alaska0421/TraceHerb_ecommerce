...
type Product = { id:number; name:string; category:string; origin:string; price:number; stock:number; sales:number; rating:number; traceCode:string; badge:string; description:string; icon:string; merchantId?:number; merchantName?:string };
type CartLine = Product & { qty:number };
type Toast = { text:string; kind?:"ok"|"warn" };
type Role = "buyer"|"seller"|"admin";
type User = { id:number; username:string; email:string; phone:string; points:number; role:Role; shopName?:string };
    ...
    {view==="shop"&&<>
      ...
      <section id="market" className="market section">
        ...
        <div className="product-grid">{filtered.map(p=><article className="product-card" key={p.id}>
          ...
          <div className="product-info"><small>{p.origin}</small><div className="merchant-name">▣ {p.merchantName||"迹本草认证商家"}</div><h3>{p.name}</h3><p>{p.description}</p><div className="rating">★★★★★ <span>{p.rating} · 已售 {p.sales}</span></div><div className="price"><strong>¥{p.price}</strong><div><button className="trace-btn" onClick={()=>setTrace(p)}>查溯源</button><button className="add-btn" onClick={()=>add(p)}>＋</button></div></div></div>
        </article>)}</div>
      </section>
...
function AuthModal({mode,setMode,user,onClose,onAuth,onLogout}:{mode:"login"|"register";setMode:(m:"login"|"register")=>void;user:User|null;onClose:()=>void;onAuth:(u:User)=>void;onLogout:()=>void}){
  const [form,setForm]=useState({username:"",email:"",phone:"",account:"",password:"",shopName:"",role:"buyer" as "buyer"|"seller"});
  const [error,setError]=useState("");
  const [loading,setLoading]=useState(false);
  const submit=async()=>{
    setLoading(true);setError("");
    try{
      const body=mode==="register"?{username:form.username,email:form.email,phone:form.phone,password:form.password,role:form.role,shopName:form.shopName}:{account:form.account,password:form.password};
...
function Merchant({user,products,setProducts,notify}:{user:User;products:Product[];setProducts:(x:Product[])=>void;notify:(x:string,kind?:"ok"|"warn")=>void}) {
  ...
  const ownedProducts=products.filter(p=>p.merchantId===user.id);
