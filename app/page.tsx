...
type Order = { id:string; item:string; amount:number; status:string; time:string; paymentMethod?:string; transactionId?:string };
...
export default function Home() {
  ...
  useEffect(()=>{if(user?.role==="buyer")fetch("/api/orders").then(r=>r.ok?r.json():[]).then((rows:{id:string;amount:number;items:{name?:string;qty?:number}[];status:string;createdAt:string;paymentMethod?:string;transactionId?:string}[])=>setOrders(rows.map(o=>({id:o.id,item:o.items.map(x=>`${x.name||"商品"} × ${x.qty||1}`).join("、"),amount:o.amount,status:o.status,time:new Date(o.createdAt).toLocaleString("zh-CN"),paymentMethod:o.paymentMethod,transactionId:o.transactionId})))).catch(()=>{})},[user]);
  ...
  const placeOrder=async()=>{
    ...
    try{
      ...
      setOrders(o=>[{id,item:cart.map(x=>`${x.name} × ${x.qty}`).join("、"),amount:+total.toFixed(2),status:"待发货",time:new Date(data.paidAt).toLocaleString("zh-CN"),paymentMethod:payMethod,transactionId:data.transactionId},...o]);
  ...
  return <main>
    ...
    {view==="orders"&&(user?<section className="dashboard section"><div className="page-title"><p className="eyebrow">MY ORDERS</p><h1>我的订单</h1><p>查看订单状态、支付凭证与溯源记录。</p></div><div className="tabs"><button className="on">全部订单</button><button>待付款</button><button>待发货</button><button>已发货</button><button>已完成</button></div><div className="orders">{orders.map(o=><article key={o.id}><div><small>订单号 {o.id} · {o.time}</small><h3>{o.item}</h3>{o.transactionId&&<small>{o.paymentMethod==="alipay_sandbox"?"支付宝沙箱":"微信模拟支付"} · {o.transactionId}</small>}</div><strong>¥{o.amount}</strong><span className={`status ${o.status}`}>{o.status}</span><button onClick={()=>notify("物流：包裹已到达上海转运中心")}>查看物流</button></article>)}{!orders.length&&<p className="empty-result">暂无订单，去本草集市挑选心仪商品吧。</p>}</div></section>:<LoginRequired onLogin={()=>{setAuthMode("login");setAuthOpen(true)}}/>)}
...
function Admin({products,setProducts,notify}:{products:Product[];setProducts:(x:Product[])=>void;notify:(x:string)=>void}) {
  const [query,setQuery]=useState("");
